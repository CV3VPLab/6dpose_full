...
def move_dino_gallery_features_to_device(
    gallery_info,
    device="cuda:0",
):
    """Move cached gallery descriptors to one device exactly once."""
    gallery_info["feats"] = gallery_info["feats"].to(device)
    for name, features in gallery_info.get(
        "auxiliary_feats",
        {},
    ).items():
        gallery_info["auxiliary_feats"][name] = features.to(device)

    total_bytes = gallery_info["feats"].numel() * gallery_info["feats"].element_size()
    total_bytes += sum(
        features.numel() * features.element_size()
        for features in gallery_info.get("auxiliary_feats", {}).values()
    )
    print(
        f"  [DINO] Gallery features: {gallery_info['feats'].device}, "
...
def load_dino_trt_session(
    onnx_path,
    trt_cache_dir=None,
    require_gpu=True,
    trt_profile_min_shapes=None,
    trt_profile_opt_shapes=None,
    trt_profile_max_shapes=None,
    ...
    if any(value is not None for value in profile_values):
        if not all(value is not None for value in profile_values):
            raise ValueError(
                "DINO TensorRT min/opt/max profiles must all be set"
            )
        trt_options.update({
            "trt_profile_min_shapes": str(trt_profile_min_shapes),
            "trt_profile_opt_shapes": str(trt_profile_opt_shapes),
            "trt_profile_max_shapes": str(trt_profile_max_shapes),
        })
    if trt_max_workspace_size is not None:
        trt_options["trt_max_workspace_size"] = int(
...
class DinoV2ExtractorTRT:
    ...
    def __init__(
        ...
        output_metas = list(sess.get_outputs())
        self._feature_output_name = next(
            (
                output.name
                for output in output_metas
        ...
        if self._feature_dim is None:
            feature_meta = next(
                output
                for output in output_metas
                if output.name == self._feature_output_name
            )
            last_dim = feature_meta.shape[-1]
            if not isinstance(last_dim, (int, np.integer)):
                raise ValueError(
                    "Cannot infer DINO feature dimension from ONNX output "
                    f"{feature_meta.shape}; use a known DINOv2 model name"
                )
            self._feature_dim = int(last_dim)
        self.use_io_binding = bool(use_io_binding)
        self.preprocess_on_gpu = bool(preprocess_on_gpu)
        self.device_id = int(device_id)
        self.device = f"cuda:{self.device_id}"

        if self.preprocess_on_gpu and not self.use_io_binding:
            raise ValueError(
                "DINO GPU preprocessing requires use_io_binding=True"
            )

        if self.use_io_binding:
            if not torch.cuda.is_available():
                raise RuntimeError("DINO I/O Binding requires CUDA")
            active = set(sess.get_providers())
            if not active.intersection({
                "TensorrtExecutionProvider",
                "CUDAExecutionProvider",
            }):
                raise RuntimeError(
                    "DINO I/O Binding requires a CUDA-capable ONNX provider"
                )
    ...
    def _encode_batch_np(self, pixel_values: np.ndarray) -> torch.Tensor:
    ...
    def _encode_np(self, pixel_values: np.ndarray) -> torch.Tensor:
        return self._encode_batch_np(pixel_values).squeeze(0)

    def _encode_batch_cuda(
        self,
        pixel_values: torch.Tensor,
    ) -> torch.Tensor:
        if not pixel_values.is_cuda:
            raise ValueError("DINO I/O Binding input must be a CUDA tensor")
        if pixel_values.dtype != torch.float32:
            raise ValueError(
                f"DINO input must be float32, got {pixel_values.dtype}"
            )
        ...
        if (
            ...
            raise ValueError(
                f"This DINO ONNX requires B={self._fixed_batch_size}, "
                f"got B={batch_size}"
            )
        ...
        io_binding.bind_output(
            name=self._feature_output_name,
            device_type="cuda",
            device_id=self.device_id,
            element_type=np.float32,
        ...
        # The processor may have produced pixel_values asynchronously on a
        # PyTorch CUDA stream. Make the bound input visible before ORT reads it.
        io_binding.synchronize_inputs()
        self._sess.run_with_iobinding(io_binding)
        io_binding.synchronize_outputs()
        return F.normalize(features, dim=-1)

    def encode_batch_rgb(self, images) -> torch.Tensor:
        images = list(images)
        if not images:
            raise ValueError("At least one DINO image is required")
        ...
        if self.use_io_binding:
            ...
            return self._encode_batch_cuda(pixel_values)

        pixel_values = self.processor(
            images=images,
            return_tensors="np",
        )["pixel_values"].astype(np.float32)
        return self._encode_batch_np(pixel_values)

    def encode_bgr(self, img_bgr: np.ndarray) -> torch.Tensor:
        rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
        return self.encode_batch_rgb([rgb]).squeeze(0)

    def encode_rgb(self, img_rgb: np.ndarray) -> torch.Tensor:
        return self.encode_batch_rgb([img_rgb]).squeeze(0)
    ...
    def encode_4rgb(self, img_rgb: np.ndarray) -> torch.Tensor:
        ...
        if self._fixed_batch_size == 1:
            features = torch.row_stack([
                self.encode_rgb(tile)
                for tile in tiles
            ])
        else:
            features = self.encode_batch_rgb(tiles)
