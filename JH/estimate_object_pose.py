...
def load_extractor(config):
    ...
    if options.get("onnx"):
        sess = load_dino_trt_session(
            ...
            trt_profile_min_shapes=options.get("trt_profile_min_shapes"),
            trt_profile_opt_shapes=options.get("trt_profile_opt_shapes"),
            trt_profile_max_shapes=options.get("trt_profile_max_shapes"),
            trt_max_workspace_size=options.get("trt_max_workspace_size"),
        )
        extractor = DinoV2ExtractorTRT(
            options["model"],
            sess,
            use_io_binding=options.get("use_io_binding", False),
            preprocess_on_gpu=options.get("preprocess_on_gpu", False),
        ...
        if options.get("warmup", True):
            ...
            for _ in range(int(options.get("warmup_runs", 1))):
                extractor.encode_batch_rgb(
                    [dummy_rgb] * int(options.get("batch_size", 1))
                )
            t1 = sync_time()
...
def retrieve_best(queryInfo, galleryInfo, extractor):
    ...
    if qfeat.is_cuda and g_feats.device != qfeat.device:
        move_dino_gallery_features_to_device(
            galleryInfo,
            qfeat.device,
        )
...
def main_object_pose_estimation():
    ...
    # Setup
    config_path = Path(__file__).resolve().parent / "ope_config.json"
    ...
    move_dino_gallery_features_to_device(gallery_info, device)
