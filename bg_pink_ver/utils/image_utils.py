...
def apply_mask(image, mask, bg_color=(0, 0, 0)):
    # Non-mask pixels use the configured RGB background, not always black.
    if mask.dtype != np.uint8:
        mask = (mask > 0).astype(np.uint8) * 255
    if isinstance(bg_color, str):
        bg_rgb = [int(v) for v in bg_color.split(",")]
    else:
        bg_rgb = [int(v) for v in bg_color]
    if len(bg_rgb) != 3:
        raise ValueError(f"Expected bg_color as RGB triplet, got: {bg_color}")
    out = np.empty_like(image)
    out[:] = np.array(bg_rgb, dtype=image.dtype).reshape(1, 1, 3)
