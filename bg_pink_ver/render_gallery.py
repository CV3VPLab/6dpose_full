...
def parse_args():
    ...
    p.add_argument("--background", default="255,0,255", type=str,
                   help="R,G,B in 0-255")
...
def parse_background_bgr(bg_color_str):
    rgb = [int(v) for v in str(bg_color_str).split(",")]
    if len(rgb) != 3:
        raise ValueError(f"Expected background as R,G,B, got: {bg_color_str}")
    return np.array([rgb[2], rgb[1], rgb[0]], dtype=np.int16)


def extract_bbox_and_crop_from_background(img_bgr, bg_color_str, margin=12, thresh=8):
    # Pink/non-black backgrounds need crop by difference from configured BG.
    bg_bgr = parse_background_bgr(bg_color_str)
    diff = np.abs(img_bgr.astype(np.int16) - bg_bgr.reshape(1, 1, 3))
    mask = np.any(diff > int(thresh), axis=2)
    ys, xs = np.where(mask)
    h, w = img_bgr.shape[:2]
    ...
    return bbox, crop_with_bbox(img_bgr, bbox)
...
def run_render_gallery(args, gaussians=None):
    ...
    for pose in tqdm(galleryPoses, desc="GS gallery rendering"):
        ...
        with torch.no_grad():
            ...
            g_crop_bbox, bgr_crop = extract_bbox_and_crop_from_background(
                rgb_bgr, args.background, margin=12, thresh=8
            )
